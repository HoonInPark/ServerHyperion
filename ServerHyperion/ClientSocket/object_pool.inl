...
namespace detail
{
	...
	SERVERHYPERION_API void* aligned_malloc(size_t size, size_t align);
	SERVERHYPERION_API void aligned_free(void* ptr);

	// Aligns n to align. N will be unchanged if it is already aligned
	SERVERHYPERION_API inline size_t align_to(size_t n, size_t align)
	{
	...
	template <typename T>
	SERVERHYPERION_API void destruct_all(ObjectPoolBlock<T>&,
	...
	template <typename T>
	SERVERHYPERION_API void destruct_all(ObjectPoolBlock<T>& t,
		typename std::enable_if<!std::is_trivially_destructible<T>::value>::type* = 0)
	{
